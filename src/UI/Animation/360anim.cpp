...
namespace {
    ...
    constexpr wchar_t kTitle[] = L"Trainer Enabled";
    constexpr wchar_t kMessage[] = L"MWCheats.asi Loaded Successfully.";
    ...
    constexpr Color kBackground = Rgb(0x1F1F1F);
    constexpr Color kCircle = Rgb(0x181818);
    constexpr Color kRingLit = Rgb(0xFFCC00);
    constexpr Color kRing = Rgb(0x333333);
    constexpr Color kLogoBorder = Rgb(0x141414);
    constexpr Color kTitleColor = Rgb(0xFFD000); // NFSMW Yellow
    constexpr Color kTextColor = Rgb(0xFFFFFF); // Pure White
    constexpr Color kGlowColor = Rgb(0xDCA400); // Tight Golden Rim Glow
    ...
    PillSample SamplePillWithGlow(float x, float y, float width, float height, float scale) noexcept {
        const float radius = height * 0.5f;
        const float cx = std::clamp(x, radius, width - radius);
        const float cy = radius;
        const float dx = x - cx;
        const float dy = y - cy;
        const float dist = std::sqrt(dx * dx + dy * dy);

        const float core = std::clamp(radius - dist + 0.5f, 0.0f, 1.0f);

        const float glowWidth = 2.5f * scale;
        float glow = 0.0f;
        if (dist >= radius - 1.0f && dist <= radius + glowWidth) {
            const float t = std::clamp((dist - (radius - 1.0f)) / (glowWidth + 1.0f), 0.0f, 1.0f);
            glow = (1.0f - t);
            glow *= std::clamp((dist - (radius - 2.0f)) / 2.0f, 0.0f, 1.0f);
        }
    ...
    TextBuffers PaintText(float scale, int left, int width, int height) {
        TextBuffers buffers;
        buffers.title.resize(static_cast<std::size_t>(width) * height, 0.0f);
        buffers.message.resize(static_cast<std::size_t>(width) * height, 0.0f);

        const int span = width - left;
        if (span <= 0) return buffers;

        const int fontHeight = -static_cast<int>(std::lround(kFontSize * scale * kSupersample));

        GdiHandle fontTitle{ CreateFontW(fontHeight, 0, 0, 0,
        ...
        if (!fontTitle) {
            fontTitle.reset(CreateFontW(fontHeight, 0, 0, 0,
                FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_TT_PRECIS,
                CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH | FF_SWISS, L"Microsoft Sans Serif"));
        }

        GdiHandle fontMessage{ CreateFontW(fontHeight, 0, 0, 0,
                                           FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_TT_PRECIS,
                                           CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH | FF_SWISS, kFont) };
        if (!fontMessage) {
            fontMessage.reset(CreateFontW(fontHeight, 0, 0, 0,
                FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_TT_PRECIS,
                CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH | FF_SWISS, L"Microsoft Sans Serif"));
        }

        if (!fontTitle || !fontMessage) return buffers;

        // Render Title Line
        {
            const Canvas canvas{ span * kSupersample, height * kSupersample };
            if (canvas) {
                SelectObject(canvas.Dc(), static_cast<HFONT>(fontTitle.get()));
                ...
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < span; ++x) {
                        ...
                        buffers.title[static_cast<std::size_t>(y) * width + left + x] = static_cast<float>(sum) / samples;
                    }
                }
            }
        }
        ...
        // Render Message Line
        {
            ...
            if (canvas) {
                ...
                SelectObject(canvas.Dc(), static_cast<HFONT>(fontMessage.get()));
                SetBkMode(canvas.Dc(), TRANSPARENT);
                SetTextColor(canvas.Dc(), RGB(255, 255, 255));
                ...
                for (int y = 0; y < height; ++y) {
                    for (int x = 0; x < span; ++x) {
                        ...
                        buffers.message[static_cast<std::size_t>(y) * width + left + x] = static_cast<float>(sum) / samples;
                    }
                }
            }
        }
    ...
    std::vector<std::uint32_t> Paint(float scale, int width, int height) {
        const TextBuffers text = PaintText(scale, static_cast<int>(std::lround(kTextLeft * scale)), width, height);
        ...
        // Chromatic Aberration Text Sampler (1px horizontal RGB shift)
        const auto sampleTextRGB = [](const std::vector<float>& buf, int x, int y, int w) -> Color {
            const auto getA = [&](int dx) -> float {
                const int sx = std::clamp(x + dx, 0, w - 1);
                return buf[static_cast<std::size_t>(y) * w + sx];
                };
            return { getA(-1), getA(0), getA(1) };
            };
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                // Title with Chromatic Shift (Yellow text)
                const Color titleRGB = sampleTextRGB(text.title, x, y, width);
                color.r += titleRGB.r * kTitleColor.r;
                color.g += titleRGB.g * kTitleColor.g;
                color.b += titleRGB.b * kTitleColor.b;

                // Message with Chromatic Shift (White text)
                const Color msgRGB = sampleTextRGB(text.message, x, y, width);
                color.r += msgRGB.r * kTextColor.r;
                color.g += msgRGB.g * kTextColor.g;
                color.b += msgRGB.b * kTextColor.b;
...
bool Xbox360Anim::Compose(int width) noexcept {
    ...
    const float scale = static_cast<float>(height_) / kHeight;
    ...
    for (int y = 0; y < height_; ++y) {
        ...
        for (int x = from; x < to; ++x) {
            const PillSample pill = x < width ? SamplePillWithGlow(static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f,
                static_cast<float>(width), static_cast<float>(height_), scale)
                : PillSample{ 0.0f, 0.0f };

            const std::uint32_t srcColor = inner_[static_cast<std::size_t>(y) * expanded_ + x];
            const Color coreColor = { static_cast<float>(srcColor >> 16 & 0xFF),
                                      static_cast<float>(srcColor >> 8 & 0xFF),
                                      static_cast<float>(srcColor & 0xFF) };

            Color finalColor;
            finalColor.r = coreColor.r * pill.coreCoverage + kGlowColor.r * pill.glowCoverage;
            finalColor.g = coreColor.g * pill.coreCoverage + kGlowColor.g * pill.glowCoverage;
            finalColor.b = coreColor.b * pill.coreCoverage + kGlowColor.b * pill.glowCoverage;

            const float totalAlpha = std::clamp(pill.coreCoverage + pill.glowCoverage, 0.0f, 1.0f);

            row[x - from] = Premultiply(Channel(finalColor.r) << 16 | Channel(finalColor.g) << 8 | Channel(finalColor.b),
                totalAlpha);
        }
    }
